...
-- prtIC2: tournaments can only be a win or a loss for a team
CONSTRAINT prtIC2 CHECK (result IN('win','loss')),
-- prtIC3: the tournament must exist
CONSTRAINT prtIC3 FOREIGN KEY (tournamentID) REFERENCES Tournaments(tournamentID)
            ON DELETE CASCADE
            DEFERRABLE INITIALLY DEFERRED,
-- prtIC4: the team must exist
CONSTRAINT prtIC4 FOREIGN KEY (teamID) REFERENCES Players(teamID)
            ON DELETE CASCADE
            DEFERRABLE INITIALLY DEFERRED,
